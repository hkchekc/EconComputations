MODULE PS4BPARA
    ...
    INTEGER, PARAMETER:: NA= INTEGER((A_MAX-A_MIN)/STEP + 1)
    INTEGER:: ZERO_LOC= INTEGER(ABS(A_MIN)/STEP + 1)
    INTEGER:: I ! ITER
...
PROGRAM PS4B ! MAIN PROGRAM
    ...
    DO WHILE (ERROR_Q>CRIT_Q)
        PRINT*, ERROR_Q, MAXVAL(Q), MAXLOC(VFUNC_CLEAN), MAXLOC(VFUNC_DEF), MAXVAL(STAT_DIST)
        PRINT*, "====================================================="
    ...
    DO SCOLIDX=1, NZ
        DO SROWIDX= ZERO_LOC, NA ! NOT POSSIBLE TO HAVE BORROWED MONEY FOR DEFAULTED PEOPL
            ...
            DO CHOICEIDX= ZERO_LOC, NA ! LOOP OVER CHOICE; AGAIN, NOT POSSIBLE TO HAVE BORROWED MONEY
                CONSUM = A_GRID(SROWIDX) + STATES(SCOLIDX) - Q(CHOICEIDX, SCOLIDX)* A_GRID(CHOICEIDX)
                IF (CONSUM > 0.) THEN
                    NEXT = RHO*SUM(EMP_MARKOV(SCOLIDX,:)*VFUNC_DEF(CHOICEIDX,:))
                    NEXT = NEXT + (1-RHO)*SUM(EMP_MARKOV(SCOLIDX,:)*VFUNC_CLEAN(CHOICEIDX,:))
                    UTIL = ((CONSUM**(1-ALPHA)-1)/(1-ALPHA))+BETA*(NEXT)
                    IF (UTIL>COND_MAX_UTIL) THEN
            ...
            DO SROWIDX=START, NA
                ...
                ELSE ! IF ALREADY DEFAULTED
                    DO EMPIDX=1, NZ
                        ...
                        PRINT*,NO_FLAG_COL,STAY_FLAG_COL
