...
PROGRAM PS5
    ...
    REAL(KIND=8):: ERROR=100. , CRIT_P=1e-1, LAST_CHOICE, CUR_CHOICE
    ...
    ENDDO
     CALL WRITE_ALL(VFUNC, LFUNC, STAT_DIST, PFUNC)
    WAGE = (1-ALPHA)*(AGG_CAP**ALPHA)*(AGG_LABOR**(-ALPHA))! FOC WRT LABOR
    INTEREST = ALPHA*(AGG_CAP**(ALPHA-1))*(AGG_LABOR**(1-ALPHA))-DELTA! FOC WRT CAPITAL
    BENEFIT = THETA*WAGE*AGG_LABOR/SUM(COHORT_POP(RETIRE_AGE:N)) ! TAX REV/RETIRED_POP
...
SUBROUTINE FIND_STAT_DIST(A_GRID, COHORT_POP, PFUNC, STAT_DIST)
    ...
    STAT_DIST_NEW = 0.
    STAT_DIST_NEW(1,1,1) = 0.2037* COHORT_POP(1)/SIZE(A_GRID) ! ALWAYS NO ASSET AT FIRST AGE,
    STAT_DIST_NEW(1,2,1) = 0.7963*COHORT_POP(1)/SIZE(A_GRID) !  BUT 2 PROD STATES
    DO AGEIDX=2, N
...
SUBROUTINE WRITE_ALL(VFUNC, LFUNC, STAT_DIST, PFUNC)
    ...
    CHARACTER(LEN=150):: FILE_NAME
        ...
        DO SROWIDX=1, NA
            WRITE(UNIT=3,FMT=*) STAT_DIST(SROWIDX, :, 1)
        ENDDO
        CLOSE(UNIT=3)

        FILE_NAME = TRIM(PATH)//"5LFUNC"
        OPEN(UNIT=4, FILE=FILE_NAME, STATUS='REPLACE') ! FOR HAVING THE X-AXIS OF PLOT
        DO SROWIDX=1, NA
            WRITE(UNIT=3,FMT=*) LFUNC(SROWIDX, :, 1)
        ENDDO
        CLOSE(UNIT=4)
END SUBROUTINE
