MODULE PS8PARA
    IMPLICIT NONE
    INTEGER:: I ! FOR ITER
    REAL(KIND=8), PARAMETER:: BETA=0.99, ALPHA=0.3, DELTA=0.025, EFF=0.3271
    ...
    TRANSPOSE(RESHAPE((/0.8507,0.1159,0.0243,0.0091,0.1229,0.8361,0.0021,&
...
SUBROUTINE VFI()
        ...
        DO WHILE (ERROR_VFI> CRIT) ! VFI
            ...
            PRINT*, ERROR_VFI, MAXLOC(ABS(VFUNC_TMP-VFUNC))
            VFUNC = VFUNC_TMP
        ENDDO
END SUBROUTINE
    ...
    DO KIDX=1,NK
        ...
        DO KPIDX=1, NK
            ...
            DO YYI=1,NY ! SHOULD USE 4 INTER2D INSTEAD OF INCREMENT LOOP FOR PARALLEL
                NEXTROW = ZZI + 2*(YYI-1)
                CALL PWL_INTERP_2D(NK, NAK, K_GRID, AK_GRID, VFUNC(:,YYI, :, ZZI), 1, SMALL_KPR, KPR, NEXTU)
...
SUBROUTINE PSEUDO_PANEL()
    ...
    USE OMP_LIB
    IMPLICIT NONE
    REAL(KIND=8), PARAMETER:: INIT_K=5.7163
